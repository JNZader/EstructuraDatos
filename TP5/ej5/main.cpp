...
void simularTurno() {
    while (!colaInicial.empty()) {
        ...
        case 'O':
            ...
            break;
        default:
            std::cerr << "Operación desconocida para DNI: " << p.DNI << std::endl;
        }
    }
}
...
void agregarPersona(const std::string& DNI, char operacion) {
    ...
    colaInicial.push(p);
    std::cout << "Persona con DNI " << DNI << " agregada a la cola inicial para la operación " << operacion << std::endl;
}
...
int main() {
    ...
    std::cout << "Atendiendo cola de depósitos:" << std::endl;
    ...
    std::cout << "Último DNI en la cola de extracciones: " << ultimoDNIEnCola(colaExtracciones) << std::endl;
